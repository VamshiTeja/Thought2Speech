...
# @Last Modified time: 2017-12-26 01:18:38
...
def build_deepSpeech2(args,
    ...
    '''
    num_splits = int(maxTimeSteps/5000)
    #count = 0
    print (inputX.shape)
    ...
    with tf.variable_scope("conv_feat") as scope:
        conv_feat = tf.get_variable("conv_feat",shape=(args.batch_size,1,5000))
...
class DeepSpeech2(object):
    ...
    def build_graph(self, args, maxTimeSteps):
        ...
        with self.graph.as_default():
            ...
            #print(self.targetY)
            self.loss = tf.reduce_mean(tf.nn.ctc_loss(self.targetY,output_fc,self.seqLengths,ctc_merge_repeated=True,time_major=False))
            ...
            num_splits = tf.to_int32(self.maxTimeSteps/5000)
            ...
            self.predictions = tf.to_int32(
                tf.nn.ctc_beam_search_decoder(output, self.seqLengths, merge_repeated=True)[0][0])
            #print(self.predictions)
            if args.level == 'cha':
                self.errorRate = tf.reduce_sum(tf.edit_distance(self.predictions, self.targetY, normalize=True))
                tf.summary.scalar("error_rate",self.errorRate)
