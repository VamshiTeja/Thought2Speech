...
# @Last Modified time: 2017-12-27 21:35:54
			...
			if(mode=='train'):
				...
				for epoch in range(num_epochs):
					...
					for batch, batchOrigI in enumerate(batchRandIxs):
						batchInputs, batchTargetSparse, batchSeqLengths = next(load_batched_data(X, labels, batch_size, mode, level))
						...
						if level == 'cha':
							_, l, pre, y, er,summary = sess.run([model.optimizer, model.loss,
								model.predictions, model.targetY, model.errorRate,model.summary_op],
								feed_dict=feedDict)
				...
				for data in load_batched_data(X, labels, batch_size, mode, level):
